...
// RUN: %clang_cc1 -triple x86_64-pc-windows-msvc19.0.24215 -std=c99 -fno-unroll-loops -O3 -mllvm -polly -mllvm -polly-position=early -mllvm -polly-process-unprofitable -mllvm -polly-use-llvm-names -emit-llvm -o -         %s | FileCheck --check-prefix=TRANS %s
...
// TRANS-LABEL: @matmul(
// TRANS: %malloccall{{[0-9]*}} = tail call dereferenceable_or_null(196608) ptr @malloc(i64 196608)
