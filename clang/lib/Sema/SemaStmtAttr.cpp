...
static Attr *handleLoopFusion(Sema &S, Stmt *St, const ParsedAttr &A,
                              SourceRange) {
  assert(A.getNumArgs() >= 1);
  ...
  return LoopFusionAttr::CreateImplicit(
      S.Context, ApplyOns.data(), ApplyOns.size(), FusedIdName, A.getRange());
}
