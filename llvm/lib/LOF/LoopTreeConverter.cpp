...
Green* GreenConverter::buildOriginalLoop(Loop* L, BasicBlock *Entry, GExpr *Cond, GreenBuilder &ParentBuilder) {
  ...
  while (!Worklist.empty()) {
    ...
    for (auto& Inst: *BB) {
      ...
      if ( auto PHIInst =  dyn_cast<llvm::PHINode>(&Inst)) {
        ...
        Stmt = Builder.addInstruction(Inst.getName(), BBCond, Operation(Operation::Nop, nullptr), {  getExpr(Inst.getOperand(0) ) }, { ResultVal }, &Inst);
      } else {
        ...
        if (llvm::isSafeToSpeculativelyExecute(&Inst)) {
          auto Op = GOpExpr::create(Operation(Operation::LLVMSpeculable, &Inst), OperandVals);
          InputsExprs[&Inst] = Op;
        ...
        if (Inst.getType()->isVoidTy()) {
          Stmt = Builder.addInstruction(Inst.getName(), BBCond, Operation(Operation::LLVMInst, &Inst), OperandVals, {}, &Inst);
        } else {
           ResultVal = GSymbol::createLLVM(&Inst);
          Stmt = Builder.addInstruction(Inst.getName(), BBCond, Operation(Operation::LLVMInst, &Inst), OperandVals, { ResultVal }, &Inst);
          MaybeEscaping.push_back(&Inst);
        }
      }
  ...
  if (L) {
    for (auto& EntryPHI : Header->phis()) {
      ...
      auto Stmt = Green::createInstruction(EntryPHI.getName(), Operation(Operation::Select, nullptr, 3), { FirstSym, EntryVal, RecurrenceVal }, { ResultVal }, &EntryPHI, nullptr);
  ...
  if (L) {
    ...
    for (auto Val : ReductionRequired) {
      auto RedVal = Ctx.createSymbol((Twine(Val->getName()) + ".lcssa").str(), Val->getType());
      auto RedInst = Builder.addInstruction((Twine(Val->getName()) + ".last").str(), nullptr, Operation(Operation::ReduceLast, nullptr), { getExpr(Val) }, { RedVal }, nullptr);
  ...
  if (L) {
    ...
    Builder.setScalarReadKillsWrites(ScalarReads, ScalarKillWrites, ScalarKillWrites, Recurrences.getArrayRef());
#endif

    //return Green::createLoop(InputDeps.size(), InputDeps, NumIntermediates, NumOutputs, LvlInsts);
    return Builder.createLoop( Header->getName(), LoopCond, &*Entry->begin(), OrigEnd, FirstSym, nullptr);
  }
  ...
  for (auto Ret : FuncReturns) {
    ...
    auto ReturnSym  = Ctx.createSymbol(".retval",Ret->getReturnValue()->getType() );
    Builder.addInstruction(".return", nullptr, Operation(Operation::ReduceReturn, nullptr), {ReturnedExpr}, {ReturnSym}, Ret);
