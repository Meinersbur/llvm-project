...
namespace lof {
  ...
  /// All fields must be declared in RedRefImpl.
    ...
    protected:
      ...
      RedRefImpl(GCommon* G, const RedRef* Parent, size_t ParentIdx) : G(G), Parent(Parent), ParentIdx(ParentIdx) {} 
    }; // class RedRefImpl
    ...
    public:
      class red_child_iterator :public llvm:: iterator_facade_base<red_child_iterator, std::random_access_iterator_tag, RedRef >     {
      public:
        friend class RedRef;
      ...
      llvm::iterator_range<red_child_iterator> children() const {
        ...
        if (Num == 0)
          return llvm::make_range(red_child_iterator(RedRef{ nullptr, this, 0 }), red_child_iterator(RedRef{ nullptr, this, 0 }));

          return llvm::make_range(red_child_iterator(getChild(0)), red_child_iterator(RedRef{nullptr, this, Num}));
      }
    ...
    public:
      ...
      void dfs_internal(
        const std::function<ResultT(const RedRef &R, ResultT &ParentResult,                   bool &ContinueChildren, bool &ContinueSiblings, bool &ContinueTree)> &PreorderCallback,
        const std::function<void   (const RedRef &R, ResultT &ParentResult, ResultT &&Result, bool &ContinueSiblings, bool &ContinueTree)> &PostorderCallback,
        ResultT &ParentResult, bool &ContinueSiblings, bool &ContinueTree) const {

        bool VisitChildren = true;
        ResultT MyResult = PreorderCallback(*this, ParentResult, MyResult, VisitChildren, ContinueSiblings, ContinueTree);
        if (!ContinueTree)
          return;
        ...
        PostorderCallback(*this, ParentResult, std::move( MyResult),    ContinueSiblings, ContinueTree);
      }
      public:
        template<typename ResultT = void>
        ResultT dfs(
          const std::function<ResultT(const RedRef &R, ResultT &ParentResult,   bool &ContinueChildren, bool &ContinueSiblings, bool &ContinueTree)> &PreorderCallback,
          const std::function<void(const RedRef &R, ResultT &ParentResult, ResultT &&Result,                         bool &ContinueSiblings, bool &ContinueTree)> &PostorderCallback) const{
          bool ContinueTree = true;
          bool ContinueSiblings = true; // Root nodes has no siblings, so without effect

          // Actually, the root's 'parent'
          ResultT RootResult;

         dfs_internal(PreorderCallback,PostorderCallback,RootResult,ContinueSiblings,ContinueTree);
