...
ArrayRef<const GreenNode *> GreenRoot::getChildren() const {
  auto Result =
      ArrayRef<const GreenNode *>((const GreenNode **)&this->Sequence, 1);
  return Result;
}
...
void GreenBlock::codegen(IRBuilder<> &Builder,
                         CodegenContext &CodegenCtx) const {
...
GreenStmt::GreenStmt(const GreenExpr *MustExecutePredicate,
                     const GreenExpr *MayExecutePredicate,
                     ArrayRef<GreenInst *> Insts, AtomDisjointSet *AtomSet)
    : GreenBlock(MustExecutePredicate, MayExecutePredicate,
                 AtomSet), // FIXME: If  MustExecute and MayExecute are not the
                           // same, the result of this atom depends on
                           // speculative execution, hence undefined; split into
                           // separate predicate that defines the atom?
      Insts(Insts) {}
...
void GreenSequence::codegen(IRBuilder<> &Builder,
                            CodegenContext &ActiveRegs) const {
  for (auto Block : Blocks)
    Block->codegen(Builder, ActiveRegs);
}

Function *
GreenLoop::codegenSubfunc(Module *M,
                          SmallVectorImpl<Value *> &UseRegArgs) const {
  auto &Context = M->getContext();
  ...
  auto Int8PtrTy = Type::getInt8PtrTy(Context);
  auto LongTy =
      Type::getInt64Ty(Context); // Depends on the bitness of the loop counter
  ...
  SmallVector<Type *, 8> ArgTypes = {/* global_tid */ Int32PtrTy,
                                     /* bound_tid */ Int32PtrTy,
                                     /* Iterations */ LongPtrTy};
  for (auto X : UseRegs) {
  ...
  Function *SubFn =
      Function::Create(FT, Function::InternalLinkage, ".parloop.subfun.", M);
  ...
  BasicBlock *PrecondThenBB =
      BasicBlock::Create(Context, "parloop.precond.then", SubFn);
  BasicBlock *InnerForCondBB =
      BasicBlock::Create(Context, "parloop.inner.for.cond", SubFn);
  BasicBlock *InnerForBodyBB =
      BasicBlock::Create(Context, "parloop.inner.for.body", SubFn);
  BasicBlock *ExitBB = BasicBlock::Create(Context, "parloop.loop.exit", SubFn);
  BasicBlock *PrecondEndBB =
      BasicBlock::Create(Context, "parloop.precond.end", SubFn);

  IRBuilder<> Builder(EntryBB);
  Value *IsLastPtr =
      Builder.CreateAlloca(Int32Ty, nullptr, "parloop.is_last.addr");
  ...
  Value *StridePtr =
      Builder.CreateAlloca(LongTy, nullptr, "parloop.stride.addr");
  ...
  if (!KmpcForStaticInit) {
    Type *Params[] = {IdentPtrTy, Int32Ty,   Int32Ty, Int32PtrTy, LongPtrTy,
                      LongPtrTy,  LongPtrTy, LongTy,  LongTy};
    FunctionType *Ty = FunctionType::get(VoidTy, Params, false);
    KmpcForStaticInit = Function::Create(Ty, Function::ExternalLinkage,
                                         "__kmpc_for_static_init_8u", M);
  }
  ...
  Value *KmpcForStaticInitArgs[] = {Ident,
                                    GlobalTidV,
                                    /* kmp_sch_static */ Builder.getInt32(34),
                                    IsLastPtr,
                                    LBPtr,
                                    UBPtr,
                                    StridePtr,
                                    Builder.getInt64(1),
                                    /* ChunkSize */ Builder.getInt64(1)};
  ...
  if (!KmpcForStaticFini) {
    ...
    FunctionType *Ty = FunctionType::get(VoidTy, Params, false);
    KmpcForStaticFini = Function::Create(Ty, Function::ExternalLinkage,
...
void GreenLoop::codegenBody(IRBuilder<> &Builder,
                            CodegenContext &ActiveRegs) const {
  ...
  if (ExecuteInParallel) {
    ...
    auto IterationsPtr =
        AllocaBuilder.CreateAlloca(LongTy, nullptr, "iterations.ptr");
    ...
    if (!KmpcForkCall) {
      Type *Params[] = {IdentTy->getPointerTo(), Int32Ty,
                        Kmpc_MicroTy->getPointerTo()};
      FunctionType *Ty = FunctionType::get(VoidTy, Params, true);
      KmpcForkCall = Function::Create(Ty, Function::ExternalLinkage,
                                      "__kmpc_fork_call", M);
    }

    Value *Task = Builder.CreatePointerBitCastOrAddrSpaceCast(
        SubFunc, Kmpc_MicroTy->getPointerTo());

    SmallVector<Value *, 8> Args = {
        Ident,
        /* Number of subfn varargs */ Builder.getInt32(1 + Params.size()), Task,
        IterationsPtr};
    for (auto Z : Params) {
    ...
    Value *LoopCondition =
        Builder.CreateICmp(CmpInst::ICMP_SLT, IV, ItersV, "loop.cond");
    BranchInst *B = Builder.CreateCondBr(LoopCondition, BodyBB, AfterBB);

    Builder.SetInsertPoint(BodyBB);
    Sequence->codegen(Builder, ActiveRegs);
    Value *IncrementedIV =
        Builder.CreateNSWAdd(IV, Builder.getInt32(1), "loop.indvar.next");
...
void GreenStmt::codegenBody(IRBuilder<> &Builder,
                            CodegenContext &ActiveRegs) const {
  for (auto Inst : Insts)
    Inst->codegen(Builder, ActiveRegs);
}

ArrayRef<const GreenNode *> GreenSet::getChildren() const {
  return ArrayRef<GreenNode *>(Val);
}

void GreenSet::codegen(IRBuilder<> &Builder,
                       CodegenContext &CodegenContext) const {
...
void GreenStore::codegen(IRBuilder<> &Builder,
                         CodegenContext &ActiveRegs) const {
...
ArrayRef<const GreenNode *> GreenCall::getChildren() const {
  return ArrayRef<GreenNode *>((GreenNode **)&Operands[0], Operands.size());
}

void GreenCall::codegen(IRBuilder<> &Builder,
                        CodegenContext &ActiveRegs) const {
...
Value *GreenConst::codegen(IRBuilder<> &Builder,
                           CodegenContext &ActiveRegs) const {
  return Const;
}

Value *GreenTrueLiteral::codegen(IRBuilder<> &Builder,
                                 CodegenContext &ActiveRegs) const {
  auto &Context = Builder.getContext();
  return ConstantInt::get(Context, APInt(1, 1));
}

Value *GreenFalseLiteral::codegen(IRBuilder<> &Builder,
                                  CodegenContext &ActiveRegs) const {
  auto &Context = Builder.getContext();
  return ConstantInt::get(Context, APInt(1, 0));
}

Value *GreenArg::codegen(IRBuilder<> &Builder,
                         CodegenContext &ActiveRegs) const {
  return Arg;
}

Value *GreenReg::codegen(IRBuilder<> &Builder,
                         CodegenContext &CodegenCtx) const {
...
Value *GreenCtrl::codegen(IRBuilder<> &Builder,
                          CodegenContext &ActiveRegs) const {
...
ArrayRef<const GreenNode *> GreenGEP::getChildren() const {
  return ArrayRef<GreenNode *>((GreenNode **)&Operands[0], Operands.size());
}

Value *GreenGEP::codegen(IRBuilder<> &Builder,
                         CodegenContext &ActiveRegs) const {
...
ArrayRef<const GreenNode *> GreenLogicOr::getChildren() const {
  return ArrayRef<GreenNode *>((GreenNode **)&Operands[0], Operands.size());
}

Value *GreenLogicOr::codegen(IRBuilder<> &Builder,
                             CodegenContext &CodegenCtx) const {
  // TODO: Shortcut-eval?
...
ArrayRef<const GreenNode *> GreenLogicAnd::getChildren() const {
  return ArrayRef<GreenNode *>((GreenNode **)&Operands[0], Operands.size());
}

Value *GreenLogicAnd::codegen(IRBuilder<> &Builder,
                              CodegenContext &CodegenCtx) const {
  // TODO: Shortcut-eval?
...
Value *GreenICmp::codegen(IRBuilder<> &Builder,
                          CodegenContext &ActiveRegs) const {
...
void neverCalled2() { GreenRoot::create(nullptr)->dump(); }
#endif
