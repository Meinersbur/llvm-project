...
#define HANDLE_NODE(ShortName, LongName) ShortName,
#define HANDLE_SUBKIND(ShortName, LongName, First, Last)                       \
  ShortName##_First = First, ShortName##_Last = Last,
...
// TOOD: Refactor-out codegen; does not need to be part of the central data
// structure. Maybe use visitor-pattern.
class CodegenContext {
public:
  DenseMap<Value *, Value *> ActiveRegs;
  DenseMap<CtrlAtom *, Value *> AtomVals;

  SmallVector<std::pair<const GreenExpr *, BasicBlock *>, 8>
      PrecomputedPredicates;
...
public:
  ...
  virtual void printText(raw_ostream &OS) const {
    printLine(OS);
    OS << '\n';
  }
...
// TODO: Integrate into GreenLoop: Everything is a loop (possible with just one
// iteration)
class GreenSequence final : public GreenNode {
private:
  // TODO: Do the same allocation trick.; Since GreenSequence is always part of
  // either a GreenRoot or GreenLoop, can also allocate in their memory
...
public:
  ...
  virtual LoopHierarchyKind getKind() const override {
    return LoopHierarchyKind::Sequence;
  }
  static bool classof(const GreenNode *Node) {
    return Node->getKind() == LoopHierarchyKind::Sequence;
  }
  ...
  iterator_range<decltype(Blocks)::const_iterator> blocks() const {
    return llvm::make_range(Blocks.begin(), Blocks.end());
  }

  static GreenSequence *create(ArrayRef<const GreenBlock *> Blocks) {
    return new GreenSequence(Blocks);
  }
...
public:
  ...
  GreenRoot *clone() const {
    auto That = create(Sequence);
    return That;
  }
  virtual ~GreenRoot(){};

  virtual LoopHierarchyKind getKind() const override {
    return LoopHierarchyKind::Root;
  }
  static bool classof(const GreenNode *Node) {
    return Node->getKind() == LoopHierarchyKind::Root;
  }
  ...
  static GreenRoot *create(const GreenSequence *Sequence) {
    return new GreenRoot(Sequence);
  }
};
...
// Instruction::mayThrow(): Could be two atoms (Or more efficient implementation
// with a flag that causes control dependencies) Loop: One atom per exiting edge
class AtomDisjointSet {
private:
...
public:
  ...
  auto atoms() const -> llvm::iterator_range<decltype(Atoms.begin())> {
    return make_range(Atoms.begin(), Atoms.end());
  }
...
protected:
  GreenBlock(const GreenExpr *MustExecutePredicate,
             const GreenExpr *MayExecutePredicate, AtomDisjointSet *Atoms)
      : GreenNode(), MustExecutePredicate(MustExecutePredicate),
        MayExecutePredicate(MayExecutePredicate), AtomSet(Atoms) {}
  GreenBlock(const GreenExpr *MustExecutePredicate,
             const GreenExpr *MayExecutePredicate, int NumAtoms)
      : GreenBlock(MustExecutePredicate, MayExecutePredicate,
                   new AtomDisjointSet(NumAtoms)) {}
...
public:
  ...
  // virtual LoopHierarchyKind getKind() const override {return
  // LoopHierarchyKind::Block; }
  static bool classof(const GreenNode *Node) {
    auto Kind = Node->getKind();
    return LoopHierarchyKind::Block_First <= Kind &&
           Kind <= LoopHierarchyKind::Block_Last;
  }
  static bool classof(const GreenBlock *) { return true; }

  // TODO: Should these be returned by children() as well?
  const GreenExpr *getMustExecutePredicate() const {
    return MustExecutePredicate;
  }
  const GreenExpr *getMayExecutePredicate() const {
    return MayExecutePredicate;
  }

  auto atoms() const -> decltype(AtomSet->atoms()) { return AtomSet->atoms(); }

  //	virtual ArrayRef <const GreenNode * const> getChildren() const override
  //{ return Stmts;}// ArrayRef<const GreenNode * const>( Stmts.data(),
  //Stmts.size()); };

  void codegen(IRBuilder<> &Builder, CodegenContext &CodegenCtx) const;
  virtual void codegenBody(IRBuilder<> &Builder,
                           CodegenContext &CodegenCtx) const = 0;

  // void codegenPredicate(IRBuilder<> &Builder, CodegenContext &ActiveRegs,
  // const std::function <void(IRBuilder<> &, CodegenContext &)> &CodegenBody)
  // const;
};
...
public:
  GreenLoop(const GreenExpr *MustExecutePredicate,
            const GreenExpr *MayExecutePredicate, GreenExpr *Iterations,
            Instruction *IndVar, GreenSequence *Sequence, Loop *LoopInfoLoop,
            AtomDisjointSet *AtomSet)
      : GreenBlock(MustExecutePredicate, MayExecutePredicate, AtomSet),
        Iterations(Iterations), IndVar(IndVar), Sequence(Sequence),
        LoopInfoLoop(LoopInfoLoop) {}
  GreenLoop *clone() const {
    auto That = create(getMustExecutePredicate(), getMayExecutePredicate(),
                       Iterations, IndVar, Sequence, nullptr, AtomSet);
    That->ExecuteInParallel = this->ExecuteInParallel;
    return That;
  }
  virtual ~GreenLoop(){};

  virtual LoopHierarchyKind getKind() const override {
    return LoopHierarchyKind::Loop;
  }
  static bool classof(const GreenNode *Node) {
    return Node->getKind() == LoopHierarchyKind::Loop;
  }
  ...
  void setExecuteInParallel(bool ExecuteInParallel = true) {
    this->ExecuteInParallel = ExecuteInParallel;
  }

  static GreenLoop *create(const GreenExpr *MustExecutePredicate,
                           const GreenExpr *MayExecutePredicate,
                           GreenExpr *Iterations, Instruction *IndVar,
                           GreenSequence *Sequence, Loop *LoopInfoLoop,
                           AtomDisjointSet *AtomSet) {
    return new GreenLoop(MustExecutePredicate, MayExecutePredicate, Iterations,
                         IndVar, Sequence, LoopInfoLoop, AtomSet);
  }

  void codegenBody(IRBuilder<> &Builder,
                   CodegenContext &ActiveRegs) const override;
};
...
public:
  GreenStmt(const GreenExpr *MustExecutePredicate,
            const GreenExpr *MayExecutePredicate, ArrayRef<GreenInst *> Insts,
            AtomDisjointSet *AtomsSet);

  virtual LoopHierarchyKind getKind() const override {
    return LoopHierarchyKind::Stmt;
  }
  static bool classof(const GreenNode *Node) {
    return Node->getKind() == LoopHierarchyKind::Stmt;
  }
  ...
  static GreenStmt *create(const GreenExpr *MustExecutePredicate,
                           const GreenExpr *MayExecutePredicate,
                           ArrayRef<GreenInst *> Insts,
                           AtomDisjointSet *AtomSet) {
    return new GreenStmt(MustExecutePredicate, MayExecutePredicate, Insts,
                         AtomSet);
  };
...
public:
  static bool classof(const GreenNode *Node) {
    auto Kind = Node->getKind();
    return LoopHierarchyKind::Inst_First <= Kind &&
           Kind <= LoopHierarchyKind::Inst_Last;
  }
  static bool classof(const GreenInst *) { return true; }

  virtual void codegen(IRBuilder<> &Builder,
                       CodegenContext &ActiveRegs) const = 0;
};
...
public:
  ...
  virtual LoopHierarchyKind getKind() const override {
    return LoopHierarchyKind::Set;
  }
  static bool classof(const GreenNode *Node) {
    return Node->getKind() == LoopHierarchyKind::Set;
  }
  ...
  static GreenSet *create(Instruction *Var, GreenExpr *Val) {
    return new GreenSet(Var, Val);
  };
...
public:
  ...
  virtual LoopHierarchyKind getKind() const override {
    return LoopHierarchyKind::Store;
  }
  static bool classof(const GreenNode *Node) {
    return Node->getKind() == LoopHierarchyKind::Store;
  }
  ...
  static GreenStore *create(GreenExpr *Val, GreenExpr *Ptr) {
    return new GreenStore(Val, Ptr);
  };
...
public:
  ...
  virtual LoopHierarchyKind getKind() const override {
    return LoopHierarchyKind::Call;
  }
  static bool classof(const GreenNode *Node) {
    return Node->getKind() == LoopHierarchyKind::Call;
  }
  ...
  static GreenCall *create(const GreenExpr *Func,
                           ArrayRef<const GreenExpr *> Operands) {
    return new GreenCall(Func, Operands);
  };
...
public:
  static bool classof(const GreenNode *Node) {
    auto Kind = Node->getKind();
    return LoopHierarchyKind::Expr_First <= Kind &&
           Kind <= LoopHierarchyKind::Expr_Last;
  }
  static bool classof(const GreenInst *) { return true; }
...
public:
  ...
  virtual LoopHierarchyKind getKind() const override {
    return LoopHierarchyKind::Const;
  }
  static bool classof(const GreenNode *Node) {
    return Node->getKind() == LoopHierarchyKind::Const;
  }
...
public:
  virtual LoopHierarchyKind getKind() const override {
    return LoopHierarchyKind::LiteralTrue;
  }
  static bool classof(const GreenNode *Node) {
    return Node->getKind() == LoopHierarchyKind::LiteralTrue;
  }
...
public:
  virtual LoopHierarchyKind getKind() const override {
    return LoopHierarchyKind::LiteralFalse;
  }
  static bool classof(const GreenNode *Node) {
    return Node->getKind() == LoopHierarchyKind::LiteralFalse;
  }
...
// Something that is set before the tree, but fixed during the tree's execution,
// i.e. an Argument or global.
...
public:
  ...
  virtual LoopHierarchyKind getKind() const override {
    return LoopHierarchyKind::Arg;
  }
  static bool classof(const GreenNode *Node) {
    return Node->getKind() == LoopHierarchyKind::Arg;
  }
...
public:
  ...
  virtual LoopHierarchyKind getKind() const override {
    return LoopHierarchyKind::Reg;
  }
  static bool classof(const GreenNode *Node) {
    return Node->getKind() == LoopHierarchyKind::Reg;
  }
...
public:
  ...
  virtual LoopHierarchyKind getKind() const override {
    return LoopHierarchyKind::Ctrl;
  }
  static bool classof(const GreenNode *Node) {
    return Node->getKind() == LoopHierarchyKind::Ctrl;
  }
  static bool classof(const GreenCtrl *) { return true; }

  void printLine(raw_ostream &OS) const override { OS << "Ctrl"; }
...
public:
  // TODO: Make private
  GreenGEP(ArrayRef<GreenExpr *> Operands)
      : Operands(Operands.begin(), Operands.end()) {}

  virtual LoopHierarchyKind getKind() const override {
    return LoopHierarchyKind::GEP;
  }
  static bool classof(const GreenNode *Node) {
    return Node->getKind() == LoopHierarchyKind::GEP;
  }
  static bool classof(const GreenGEP *) { return true; }

  ArrayRef<GreenExpr *> getOperands() const { return Operands; }
  GreenExpr *getBase() const {
    assert(Operands.size() >= 1);
    return Operands[0];
  }
  ArrayRef<GreenExpr *> getIndices() const {
    return ArrayRef<GreenExpr *>(Operands).drop_front(1);
  }
  ...
  static GreenGEP *create(ArrayRef<GreenExpr *> Operands) {
    return new GreenGEP(Operands);
  }
...
public:
  GreenICmp(ICmpInst::Predicate Predicate, GreenExpr *LHS, GreenExpr *RHS)
      : Operands{LHS, RHS}, Predicate(Predicate) {}

  virtual LoopHierarchyKind getKind() const override {
    return LoopHierarchyKind::ICmp;
  }
  static bool classof(const GreenNode *Node) {
    return Node->getKind() == LoopHierarchyKind::ICmp;
  }
  ...
  static GreenICmp *create(ICmpInst::Predicate Predicate, GreenExpr *LHS,
                           GreenExpr *RHS) {
    return new GreenICmp(Predicate, LHS, RHS);
  }
};
...
protected:
  GreenLogicOr(ArrayRef<const GreenExpr *> Operands)
      : Operands(Operands.begin(), Operands.end()){};

public:
  virtual LoopHierarchyKind getKind() const override {
    return LoopHierarchyKind::LogicOr;
  }
  static bool classof(const GreenNode *Node) {
    return Node->getKind() == LoopHierarchyKind::LogicOr;
  }
  ...
  Value *codegen(IRBuilder<> &Builder,
                 CodegenContext &CodegenCtx) const override;
...
protected:
  GreenLogicAnd(ArrayRef<const GreenExpr *> Operands)
      : Operands(Operands.begin(), Operands.end()){};

public:
  virtual LoopHierarchyKind getKind() const override {
    return LoopHierarchyKind::LogicAnd;
  }
  static bool classof(const GreenNode *Node) {
    return Node->getKind() == LoopHierarchyKind::LogicAnd;
  }
